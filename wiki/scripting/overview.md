...
Planners 使用标准 JavaScript 作为脚本语言，当前统一通过 SE（ScriptEngine）的 GraalJS 实现执行脚本。
...
| 运行实现 | GraalJS |
| 依赖 | 需要安装 ScriptEngine 前置 |

旧文档中的 Kether、Fluxon、Nashorn 说明已经不适用于当前版本。
...
Planners 脚本就是标准 JavaScript。如果你写过网页 JS，基础语法可以直接沿用。
...
| 状态 | [状态 API](functions-state.md) | `stateAPI.attach`, `stateAPI.detach`, `stateAPI.remove`, `stateAPI.has`, `stateAPI.getLayer` |
...
| PlaceholderAPI | [PlaceholderAPI 函数](functions-placeholder.md) | `papi` |
