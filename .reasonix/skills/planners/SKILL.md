...
| `level` | int | 技能等级 |
| `skill` | ImmutableSkill | 技能对象（`skill.id`、`skill.name`） |
...
```javascript
// 变量（读取 variables 中定义的公式项）
var power = getVar("power")       // 缓存：首次计算后复用
var hp = calcVar("max_hp")        // 每次重新计算
...
按键 → CombinedAnalyzer → BackpackAPI.getSkillByKey()
  ...
  → ② 释放前条件循环 verify            → 失败：PreConditionFailed + feedback
     (按 priority: CD=100 → MP=200 → 自定义...)
  → ③ PlayerSkillCastEvent.Pre        → 取消：CANCEL_WITH_EVENT
  → ④ 释放前条件循环 consume           → 扣除资源（冷却/MP/自定义）
  → ⑤ Hook 拦截？（SkillInputExecHook）
      ├─ 是 → INTERCEPTED（等待 resume()）
      └─ 否 → skill.execute() → Post 事件 → SUCCESS
```

### 释放前条件 (CastPreCondition)

条件校验和消耗通过可插拔的 `CastPreCondition` 接口实现，按 `priority` 升序执行。

| 内置条件 | key | priority | 说明 |
|----------|-----|----------|------|
| `PlayerStatePreCondition`（FightCore） | `player-state` | 30 | 必须处于 COMBAT 状态才能释放 |
| `WeaponFormPreCondition`（FightCore） | `weaponform` | 50 | 技能所属 job 须在当前武器形态的 `planners.job` 列表中 |
| `CooldownPreCondition` | `cooldown` | 100 | 校验冷却，consume 设置冷却 |
| `MagicPointPreCondition` | `mp` | 200 | 校验 MP，consume 扣除魔法值 |
| `StaminaPreCondition`（FightCore） | `stamina` | 200 | 校验体力，consume 扣除体力值 |

FightCore 条件通过 fight-core configpack 的 weaponform YAML 配置关联：

```yaml
# configpack/animations/weaponforms/longsword.yml
__options__:
  planners:
    job:
      - swordsman
...
未配置 `planners.job` 的武器形态不限制职业。

扩展方式：

```kotlin
// 注册自定义条件
PlannersAPI.registerCastPreCondition(myCondition)
// 替换失败反馈（默认 sendMessage）
PlannersAPI.setCastPreConditionFeedback(myFeedback)
```

技能 YAML 中定义对应 `variables` 即可自动生效（如 `stamina: 10`）。

### ExecutableResult 返回值 (sealed class)

| 子类 | 含义 |
|------|------|
| `Success` | 释放成功 |
| `Cooling` | 冷却中（向后兼容） |
| `MagicPointInsufficient` | 魔法值不足（向后兼容） |
| `CancelledWithEvent` | 被 Check/Pre 事件取消 |
| `Intercepted(cause)` | 被 Hook 接管，等待异步 resume() |
| `PreConditionFailed(failure)` | 释放前条件校验失败，携带 `CastPreConditionResult.Failure` |
